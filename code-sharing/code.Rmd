...
Time <- data_xz$Timestamp
Y_leads <- purrr::map(1:24, ~dplyr::lead(data_y$turbidity_downstream, 
                                      n = .x))
names(Y_leads) <- paste("Y_", 1:24, sep = "")

Y_leads <- Y_leads %>%
    tibble::as_tibble()

X <- data_xz %>% dplyr::pull(turbidity_upstream)

func_ccf <- function(y, x){
    cor(y,x, use = "complete.obs")
  }

max_lag_ccf <- apply(Y_leads, 2, FUN = func_ccf, x = X) %>% 
  which.max()
